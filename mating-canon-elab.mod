...
test_elab X :-
  example X F,
  % check_mating pseudo-inlined here, without final join
  nnf conj- disj- F F',
  lkf_entry ((mating zero nil Mating') <c> (can 0 Elab)) F',
  % Output
  term_to_string ((mating zero nil Mating') <c> (can 0 Elab)) String,
  print "Certificate = \n",
  print String,
  print "\n".
